...
function NavItem({ item, isChild = false, onNavigate }) {
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `${padding} group flex items-center gap-3 rounded-lg text-sm transition-colors ${
          isActive
...
function filterSections(sections, hasAllPermissions) {
  return sections
    .map((section) => {
      ...
      if (Array.isArray(section.items) && section.items.length > 0) {
        const filteredItems = section.items.filter((item) => {
          ...
          if (requiredItemPerms.length === 0) {
            return true;
          }
          return hasAllPermissions(requiredItemPerms);
        });
...
function SidebarComponent({ brand = DEFAULT_BRAND, sections = DEFAULT_SECTIONS, onNavigate, onLogout }) {
  const location = useLocation();
  const { hasAllPermissions } = useAuth();
  const filteredSections = useMemo(
    () => filterSections(sections, hasAllPermissions),
    [sections, hasAllPermissions],
  );
      ...
      <div className="mt-auto space-y-4 border-t border-gray-100 p-5">
        ...
        {onLogout ? (
          <button
            type="button"
            onClick={onLogout}
            className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gray-900 px-4 py-2 text-sm font-medium text-white transition-opacity hover:opacity-90"
          >
